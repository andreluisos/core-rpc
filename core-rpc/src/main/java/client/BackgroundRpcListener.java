 ...
 *
 * <p>Messages are read using {@link ExecutorService} An infinite task is submitted to the service
 * which constantly waits for and reads messages from the attached stream It may be stopped using
 * {@link #stop()}
 *
 * <p>Messages are deserialized using {@link ObjectMapper} passed in the constructor
 *
 * <p>This class supports 3 types of callbacks required by {@link RpcListener}: - {@link
 * RpcListener.RequestCallback} - {@link RpcListener.ResponseCallback} with a specific id to respond
 * to - {@link RpcListener.NotificationCallback}
 *
 * <p>Every {@link RpcListener.RequestCallback} is notified when request comes Request callback is
 * run on the same thread as the one used for reading, meaning long running callbacks will block
 * reading. For longer tasks async request callback should be implemented.
 *
 * <p>Same holds true for {@link RpcListener.NotificationCallback} and {@link
 * RpcListener.RequestCallback}, with the exception that {@link RpcListener.RequestCallback} will
 * only be called for messages with matching id
 *
 * <p>Example:
 *
 * <pre>{@code
...
public final class BackgroundRpcListener implements RpcListener {
     ...
     */
    public BackgroundRpcListener(
            ExecutorService executorService, ObjectMapper responseObjectMapper) {
        Objects.requireNonNull(
                executorService, "executorService must be provided to enable background work");
        Objects.requireNonNull(
                responseObjectMapper, "responseObjectMapper must be provided to deserialize");
     ...
     *
     * <p>The listening never stops, unless {@link #stop()} is used Listener may be restarted after
     * stopping
     *
     * <p>Calling start multiple times has no effect - only first one is considered It can be called
     * again only after calling {@link #stop()}
     *
    ...
    /**
     * Stops current listener by killing the task submitted to executor If listener is currently not
    ...
    /**
     * Prepares a {@link RpcListener.ResponseCallback} Once a response with message id equal to the
     * id passed to this method comes, the {@link RpcListener.ResponseCallback} will be notified. It
     * will then be removed, meaning it will not be called multiple times.
     *
     * <p>Only single {@link RpcListener.ResponseCallback} is supported per message id. If multiple
     * callbacks are required, consider delegating to them through a single callback.
     *
    ...
    /**
     * Prepares a {@link RpcListener.NotificationCallback} It will be notified for any notification
     * that comes through to this listener
     *
     * <p>Since {@link RpcListener.NotificationCallback} will be notified whenever any notification
     * comes, on the same thread that is used for listening, these callbacks should not do heavy
     * work. If heavy work is required, it should be run on a different thread (that is not handled
     * by this class).
     *
     * <p><b>null</b> is supported and can be used to effectively remove current callback
     *
    ...
    /**
     * Prepares a {@link RpcListener.RequestCallback} It will be notified for any request that comes
     * through to this listener
     *
     * <p>Neovim requires that requests are handled immediately, but this class does not offer such
     * functionality. Other classes should be used to send a message.
     *
     * <p>Since {@link RpcListener.RequestCallback} will be notified whenever any request comes, on
     * the same thread that is used for listening, these callbacks should not do heavy work. If
     * heavy work is required, it should be run on a different thread (that is not handled by this
     * class).
     *
     * <p><b>null</b> is supported and can be used to effectively remove current callback
     *
    ...
    private void listenForMessages(InputStream inputStream) throws IOException {
        ...
        while ((readNode = objectReader.readTree(inputStream)) != null) {
            ...
            switch (messageType) {
                case REQUEST:
                    var requestMessage =
                            responseObjectMapper.treeToValue(arrayNode, RequestMessage.class);
                ...
                case RESPONSE:
                    var responseMessage =
                            responseObjectMapper.treeToValue(arrayNode, ResponseMessage.class);
                    if (responseCallbacks.containsKey(responseMessage.getId())) {
                        log.debug(
                                "Notifying response callback for id({}) with: {}",
                                responseMessage.getId(),
                                responseMessage);
                        responseCallbacks
                                .get(responseMessage.getId())
                                .responseReceived(responseMessage.getId(), responseMessage);
                        responseCallbacks.remove(responseMessage.getId());
                    }
                    break;
                case NOTIFICATION:
                    var notificationMessage =
                            responseObjectMapper.treeToValue(arrayNode, NotificationMessage.class);
